...
lat = ncread([data_dir, fname1],'latitude');
lon = ncread([data_dir, fname1],'longitude');
t = ncread([data_dir, fname1],'temperature'); %Conservative Temperature
s = ncread([data_dir, fname1],'salinity'); %Absolute Salinity
divenum = ncread([data_dir, fname1],'dives');
time = ncread([data_dir, fname1],'time'); %seconds since 1970-01-01
secperday = 86400;
...
p_ox = ncread([data_dir, fname2],'pressure'); %dbar
ox = ncread([data_dir, fname2],'oxygen'); %micromoles/L
time_ox = ncread([data_dir, fname2],'time'); %seconds since 1970-01-01
